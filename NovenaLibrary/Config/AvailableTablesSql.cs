...
namespace NovenaLibrary.Config
{
    public static class AvailableTablesSql
    {
        public static readonly Dictionary<DatabaseType, string> availableTablesSql = new Dictionary<DatabaseType, string>
        {
            ...
            {DatabaseType.MsSqlServer,  "SELECT table_name FROM sp_table_privileges WHERE grantee = '{0}' AND privilege = 'SELECT';"},
            ...
            {DatabaseType.MsAccess,     "SELECT name FROM MSysObjects WHERE Type = 1 AND Flags = 0"}
        };
    }
}
